...
exports.editUser = async (req, res) => {
    try {
        ...
        const { user_Name, user_Email, country_code, mobile_no, user_type  } = req.body;

        // Validate request data
        if (!user_Name || !user_Email || !country_code || !mobile_no || !user_type) {
        ...
        const updatedUser = await user.findOneAndUpdate(
            { _id: userId },
            { $set: { user_Name, user_Email, country_code, mobile_no, user_type} },
            { new: true }
        );
...
// Get total count of users, vendors, and item/list
exports.getTotalCounts = async (req, res) => {
    try {
        ...
        // const totalItemList = await list.countDocuments({ deleteFlag: false }); // ✅ Count only active events
        const totalItemList = await list.countDocuments();
